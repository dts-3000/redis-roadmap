    ...
    <main className="min-h-screen bg-slate-50 p-6 text-slate-900">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white rounded-3xl p-6 border shadow-sm h-[600px] flex flex-col">
          ...
          <div className="overflow-y-auto space-y-1">
            {Object.keys(musicData).sort().map(band => (
              <button key={band} onClick={() => setSelectedBand(band)} className={`w-full p-4 rounded-xl text-left font-bold ${selectedBand === band ? 'bg-slate-900 text-white' : 'hover:bg-slate-50'}`}>{band}</button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border shadow-sm h-[600px] flex flex-col">
          ...
          <div className="overflow-y-auto space-y-1">
            {selectedBand ? musicData[selectedBand].map(song => (
              <button key={song} disabled={votingSlip.includes(`${selectedBand} - ${song}`) || votingSlip.length >= 5} onClick={() => setVotingSlip([...votingSlip, `${selectedBand} - ${song}`])} className="w-full p-4 rounded-xl text-left font-bold border-2 border-transparent hover:border-yellow-400 disabled:opacity-20">{song}</button>
        ...
        <div className="bg-slate-900 rounded-[2.5rem] p-8 h-[600px] text-white flex flex-col shadow-2xl">
          ...
          <button disabled={votingSlip.length === 0} onClick={async () => { await submitFinalVotes(votingSlip); setVotingSlip([]); await init(); alert("Votes In!"); }} className="w-full bg-yellow-400 text-slate-900 p-5 rounded-2xl font-black uppercase mt-6 hover:bg-yellow-300 disabled:opacity-10">Submit Votes</button>
        </div>
      </div>
    </main>
  );
